...
| `data/visible/` | The analyst's dataset: receipts (including voids and refunds), book-inventory snapshots, procurement invoices, price history, promotions, cost sheets, write-offs with stock-count corrections, calendar, weather — realistically imperfect records; `documents/ACCOUNTING.md` defines what reconciles with what |
| `data/hidden/` | The answer key: customer parameters, demand modifiers and tilts, cost paths, budget paths, event log, the hidden-demand ledger (64k unmet-demand events across four causes), owner forecasts, the believed/realized/oracle profit triptych (pre- and after-tax), and the ledger of every injected data defect (`imperfections.csv`) |
| `data/scenarios/<name>/` | CRN-twin scenario arms of the same year under edited macro scripts — war, typhoon, VAT reform, tax rebate, staffing — each with its own visible/hidden split, plus `comparison.csv` across arms; run `uv run python generate_dataset.py --all-scenarios` |
