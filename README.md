...
| `analyses/analysis_workbook.py` | Marimo workbook: builds a DuckDB database from the one-year baseline, then EDA and a senior-analyst deep dive narrated for the owner (elasticities, promotion lift, stockout/spoilage economics, customer analytics, ML forecasting) — survey depth across Layers 0–4, predates the full-depth series below and not yet retrofitted to the three-year arm |
...
## Roadmap
  ...
  8. **Remaining / deferred:** retrofit `analysis_workbook.py` (still one-year-only) to the three-year arm; a scenario-configurator front end that would let a user pick a scenario, generate its data, and open its graded notebooks in one flow.
