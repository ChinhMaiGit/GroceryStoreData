...
from .params import BASKET_SHARE, CATS, PHASE1, PHASE3, PHASE4
...
def run_year(
    ...
    w, p3 = world, PHASE3
    p5 = w.p5
    # freezer/operational_hazard (P5 §7) is a repeatable equipment-failure
    # event: events.py's composer can pass either the original single dict
    # (the reference 3y_baseline shape) or a list of dicts for several
    # occurrences. Normalize once, here, so both read sites below just loop.
    freezer_events = []
    if p5:
        _fz_spec = p5["freezer"]
        freezer_events = _fz_spec if isinstance(_fz_spec, list) else [_fz_spec]
    ...
    expanded = False
    expansion_t = None
    # bigger_store / upgrade_infrastructure (grocery_sim's own addition):
    # same trigger pattern as the hire expansion, independently gated
    expanded_shelf = False
    shelf_t = None
    expanded_infra = False
    infra_t = None
    infra_scale = 1.0        # multiplies nightly spoilage probability
    ...
    for day in w.cal.itertuples():
        ...
        if p5:
            ...
            if expansion_t == t:
                ...
                expanded = True
            # bigger_store: extra shelf capacity, stacks multiplicatively
            # with whatever shelf_mult the hire expansion already set
            if shelf_t == t:
                cash -= p5["finance"]["shelf_capex"]
                re_balance -= p5["finance"]["shelf_capex"]
                month_acc["capex"] += p5["finance"]["shelf_capex"]
                shelf_mult *= p5["finance"]["shelf_mult_bonus"]
                expanded_shelf = True
            # upgrade_infrastructure: a better freezer/cold chain scales
            # every SKU's nightly spoilage probability down from here on
            if infra_t == t:
                cash -= p5["finance"]["infra_capex"]
                re_balance -= p5["finance"]["infra_capex"]
                month_acc["capex"] += p5["finance"]["infra_capex"]
                infra_scale = p5["finance"]["infra_spoilage_mult"]
                expanded_infra = True
            # the freezer dies overnight (P5 §7): the frozen aisle and part
            # of the dairy case go in the bin at opening, the repairman bills
            # — repeatable: one independent failure per entry in freezer_events
            for _fz in freezer_events:
                if t != _fz["t"]:
                    continue
                for i in w.listed:
        ...
        for i in w.listed:
            if w.spoil_daily[i] > 0 and inv[i] > 0:
                ...
                _lam_eff = min(p3["spoil_daily_cap"],
                               w.spoil_daily[i] * w.spoilf[w.cat_of[i]][t - 1] * infra_scale)
                sp = g.binomial(
                    n = int(inv[i]),
                    p = _lam_eff,
                )
                ...
                if c == "Frozen Foods":
                    for _fz in freezer_events:
                        if _fz["t"] <= t < _fz["t"] + _fz["cap_days"]:
                            # multiple simultaneous repair windows compound
                            # the capacity loss rather than overwrite it
        ...
        if nxt.month != day.date.month or t == n_days:
            ...
            # events.tax_raise: a direct levy on gross revenue, unlike VAT
            # never passed through to shelf prices — a real cash cost here
            revenue_tax = w.revenue_tax_rate[t - 1] * month_acc["revenue"]
            cash -= wages + payroll + utils + storage + rent + vat_due + revenue_tax
            interest = credit * PHASE3["credit_apr"] / 12
            cash -= interest
            month_result = (month_acc["revenue"] - month_acc["procurement"]
                            - rent - wages - payroll - utils - storage
                            - month_acc["flyers"] - vat_due - revenue_tax - interest
                            - month_acc["repairs"])
            draw = 0.0
            profit_tax_paid = 0.0
            if p5:
            ...
            if (p5 and not expanded and expansion_t is None
                ...
                expansion_t = t + 1
            # bigger_store and upgrade_infrastructure: same trigger pattern,
            # independently gated, each firing once its own threshold clears
            if (p5 and not expanded_shelf and shelf_t is None
                    and p5["finance"].get("shelf_threshold") is not None
                    and re_balance >= p5["finance"]["shelf_threshold"]
                    and cash - tax_reserve >= p5["finance"]["shelf_capex"]):
                shelf_t = t + 1
            if (p5 and not expanded_infra and infra_t is None
                    and p5["finance"].get("infra_threshold") is not None
                    and re_balance >= p5["finance"]["infra_threshold"]
                    and cash - tax_reserve >= p5["finance"]["infra_capex"]):
                infra_t = t + 1
            row = {
