...
# Standalone-package layout: everything the library needs (SKUs.xlsx, the
# default output directory) lives inside the package itself, not the
# monorepo it was extracted from. A running Simulation always passes an
# explicit `out=` to export()/validate() rather than relying on this
# default; it exists only as a fallback for direct module use.
...
PHASE4 = {
    ...
    # events.tax_raise (grocery_sim's own addition, not part of the original
    # datagen design): a direct levy on gross revenue, unlike VAT (passed
    # through to shelf prices) or profit tax (only bites in a profitable
    # year) — modeled on real gross-receipts taxes some jurisdictions levy
    # on retailers. Zero at baseline; a scenario sets a rate from a date.
...
PHASE5 = {
    ...
    # P5 §4: retained earnings and the expansion decision. Three independent
    # investments share the same trigger logic (RE and spendable-cash
    # thresholds, single-fire, executes the first of the following month);
    # only their own threshold/capex/effect differ. Hiring is the largest
    # commitment (an ongoing wage, not just capex) and is calibrated to the
    # published autumn-2026 crossing; the shelf and infrastructure upgrades
    # are smaller, capex-only commitments and are calibrated lower.
    "finance": {
        ...
        # potential_investment.bigger_store: extra shelf capacity, stacks
        # multiplicatively with the hiring expansion's own shelf_mult
        "shelf_threshold": 18_000.0,
        "shelf_capex": 6_000.0,
        "shelf_mult_bonus": 1.15,
        # potential_investment.upgrade_infrastructure: a better freezer/cold
        # chain, scaling every SKU's nightly spoilage probability down
