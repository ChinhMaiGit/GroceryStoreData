...
def _(DATA, ROOT, duckdb, pl):
    ...
    # the baseline is one arm among the scenarios (P4 §1)
    _visb = DATA / "scenarios" / "baseline" / "visible"
    ...
    _cal = pl.read_csv(source = _visb / "calendar.csv")
...
def _(DATA, ROOT, duckdb, pl):
    ...
    _vis = DATA / "scenarios" / "baseline" / "visible"
...
def _(mo):
    ...
    this notebook runs from the raw files in `data/scenarios/baseline/visible/`
    (plus `SKUs.xlsx`, the product catalog). That underlying dataset is itself
